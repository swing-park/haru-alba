    ...
    <>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-xl font-bold dark:text-slate-100">내 공고 관리</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {jobs.map((job) => (
              <div key={job._id} className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-slate-700">
                ...
                <h2 className="font-semibold mb-1 dark:text-slate-100">{job.title}</h2>
                <p className="text-sm text-gray-500 dark:text-slate-400 mb-3">{job.location.address} · {job.startTime}~{job.endTime}</p>
                <div className="flex gap-2">
                  <Link
                    href={`/employer/jobs/${job._id}/applications`}
                    className="text-sm border border-orange-500 text-orange-500 px-3 py-1.5 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20"
                  >
                    지원자 보기
                  </Link>
                  {job.status === 'open' && (
                    <button
                      ...
                      className="text-sm border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-400 px-3 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700"
                    >
                      마감하기
                    </button>
                  )}
